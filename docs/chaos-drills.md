...
| `service-killed` | Stop `telemetry` | The map freezes with its last marker; everything else continues | Console *Live map* shows *Frozen · reconnecting*; rental creation in Grafana | Holds. Rental created (200, 453 ms), listing 200 |
...
| `risk-pricing-stopped` | Stop `risk-pricing` | Rentals priced on the last table; unscored riders pay the base rate | `dependency_degradations_total{dependency="risk-pricing"}` | Holds. Rental created (200, 117 ms); fallback counter 0 → 1 |
| `billing-stopped` | Stop `billing` | The rental list renders every row and says invoices are missing | Console rental list; gateway 5xx for `/api/invoices` | Holds. Listing 200 with both rows, `missing: ["invoices", …]` |
