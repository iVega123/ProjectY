...
| `redis-down` | Shared Redis timeout | `Idempotency-Key` mutations and revocation checks refuse closed; rate limiting is unaffected, having its own Redis since [#193](https://github.com/iVega123/ProjectY/issues/193); login, refresh and logout continue, and nobody is logged out ([#59](https://github.com/iVega123/ProjectY/issues/59)) | Gateway 503 traces | Measured before #193, when the limiter still shared this Redis: every creation refused at ~252 ms, and no 429s because the limiter failed open too. Not re-measured since the split |
| `rate-limit-redis-down` | Rate-limit Redis timeout | Rate limiting degrades open; everything else continues, rental creation included | `gateway_ratelimit_degraded_total` | Not measured yet |
