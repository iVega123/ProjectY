...
|---|---|---|---|---|---|
| **Redis** (shared) | Revocation-checked rental creation and every `Idempotency-Key` mutation (fail **closed**, 503). Live position updates. | Rate limiting, which has its own Redis ([ADR 0026](adr/0026-redis-durability-follows-the-consumer.md)). Authenticated reads; JWT verification from cached JWKS. Login, refresh, logout and revoke-all, because sessions and their revocations live in CockroachDB (ADR 0017): **nobody is logged out**, and a revocation made during the outage is copied back to the denylist within 5 s of Redis returning. | Gateway 503 traces; identity log `denylist não regravada` | gateway `fails_closed_when_revocation_cannot_be_checked`; rental-core `IdempotencyFailClosedTests`; identity `TestWithRedisDownNobodyIsLoggedOutAndLogoutStillEnds`, `TestAnUnreachableRedisDoesNotHoldUpLogout` | `redis-down` |
| **Rate-limit Redis** | Rate limiting (fails **open**: ordinary traffic continues unlimited, without the remaining-token header). | Everything else, rental creation included. | `gateway_ratelimit_degraded_total` | gateway `fails_open_and_exposes_the_degradation_counter` | `rate-limit-redis-down` |
