...
| Cross-service HTTP dependency | Rental calls have a one-second HTTP deadline. Transport failures and upstream 5xx become 503, with a distinct refusal counter and a trace degradation tag. Business 4xx remain client errors. | Controller tests cover wrapped 503 versus 404 / business rejection. |
| RabbitMQ | The transactional outboxes retain unpublished events and retry after recovery. Consumers use durable bounded retries and DLQ. | #69 / #154, outbox integration tests. This is the active broker; it is not Kafka evidence. |
...
```sh
dotnet test services/rental-core/RentalCoreTests/RentalCoreTests.csproj --filter FullyQualifiedName~DependencyFailureTests
