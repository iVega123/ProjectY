...
export default function Console() {
  ...
  return <div className="shell">
    ...
    <main><header><div className="breadcrumb">PROJECT Y <span>/</span> OPERATIONS CONSOLE</div><div className="header-state"><i className={user?'dot':'dot muted'}/>{user?'Session connected':'Sign-in required'}</div></header>
      ...
      {tab==='Live map' && <section className="workspace"><div className="map-panel panel"><LiveMap position={position}/><div className="map-label"><i className={connection==='Connected'?'dot':'dot muted'}/>{connection}</div>{!position && <div className="map-empty">{selected?'Waiting for the rider’s first position':'Select an active rental to follow its position'}</div>}<div className="map-bottom"><strong>{position?position.latitude.toFixed(5)+', '+position.longitude.toFixed(5):'No position received'}</strong><span>{age===null?'Positions arrive over a live connection':age+'s since last update'}</span></div></div><aside className="detail panel"><div className="eyebrow">LIVE RENTAL</div><h2>Follow the ride.</h2><p>Select a rental from your account. The marker moves only when a real position arrives.</p><label>Active rental<select value={selected} onChange={e=>setSelected(e.target.value)}><option value="">Choose a rental</option>{rentals.filter(r=>!r.actualEndDate).map(r=><option key={r.rentalId} value={r.rentalId}>{r.motocycleLicencePlate} · {r.rentalId.slice(-6)}</option>)}</select></label><div className="detail-row"><span>Presence</span><strong>{presence} connected</strong></div><div className="detail-row"><span>Last position</span><strong>{age===null?'Awaiting data':age+'s ago'}</strong></div><button disabled={!selected||connection!=='Connected'} onClick={locate}>Send my location ↗</button><p className="caption">Uses your device’s location with your permission. When tracking disconnects, the last marker stays in place.</p><button className="secondary" disabled={!user} onClick={()=>refresh().catch(e=>setError(String(e)))}>Refresh rentals ↻</button></aside></section>}
